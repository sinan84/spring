...
public class TerminalOperations {
    public static void main(String[] args) {
        ...
        Role r = ArrayList.createRoleList().stream().filter(role -> role.getDescription().equals("Employee")).findFirst().get();
        System.out.println(r.getId());

        Role r2 = ArrayList.createRoleList().stream().filter(role -> role.getDescription().equals("Employee")).findAny().get();
